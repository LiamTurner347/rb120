...
class PokerHand
  ...
  def royal_flush?
    return false if @rank_count != {10 => 1, "Jack" => 1, "Queen" => 1, "King" => 1, "Ace" => 1}
    flush?
  end
  ...
  def flush?
    @cards.count { |card| card.suit == @cards[0].suit } == 5
  end

  def straight?
    sorted_values = @cards.sort.map do |card|
      card.value
    end
    (0..3).each do |idx|
      return false if sorted_values[idx + 1] - sorted_values[idx] != 1
    end
    true
  end
  ...
  def two_pair?
    (@rank_count.values.count { |value| value == 2 }) == 2
  end
